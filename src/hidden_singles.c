...
void find_hidden_single(Cell **p_cells, HiddenSingle *p_hidden_singles,int *p_counter) {
    int *hidden_singles_values = (int*) malloc(BOARD_SIZE * sizeof(int));
    int numHiddenValue;
    numHiddenValue = find_hidden_single_values(p_cells, hidden_singles_values);
    for (int i = 0; i < numHiddenValue; i++) {
        for (int j = 0; j < BOARD_SIZE; j++) {
            ...
            if (!check) continue;
            if (is_candidate(p_cells[j], hidden_singles_values[i])) {
                p_hidden_singles[*p_counter].p_cell = p_cells[j];
                p_hidden_singles[*p_counter].value = hidden_singles_values[i];
                (*p_counter)++;
            }
        }
    }
    free(hidden_singles_values);
}
